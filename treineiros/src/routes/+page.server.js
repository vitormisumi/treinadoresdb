...
async function lastMatches() {
    const [rows, fields] = await accessPool().query(`
        SELECT m.match_id, m.date_time, c.name AS competition, ht.team_id, ht.name AS home_team_name, at.team_id, at.name AS away_team_name, m.home_score, m.away_score, hc.name AS home_coach, hc.nickname AS home_coach_nickname, ac.name AS away_coach, ac.nickname AS away_coach_nickname 
        FROM matches AS m 
